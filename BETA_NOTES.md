...
### 주요 변화
- 펌 게시글 원문 검사를 추가했습니다.
  - 디시인사이드가 실제 펌 게시글로 표시하는 글은 바깥 글과 함께 원문의 제목·본문·작성자가 넣은 링크와 이미지 정보도 기존 필터 및 AI 필터에서 검사합니다.
  - 제목에 `(펌)`을 직접 적은 일반 글은 펌 전체 차단 대상으로 잘못 처리하지 않습니다.
  - 원문에서 위반이 발견돼도 현재 갤러리에 올라온 바깥 펌 글과 그 작성자를 처리하며, 화이트리스트로 보호한 작성자는 제외합니다.
  - 원문이 삭제됐거나 잠시 불러오지 못해도 바깥 글 검사는 계속하고, 원문 확인 실패만으로 차단하지 않습니다.
- 펌 전용 운영 설정을 추가했습니다.
  - `게시물 차단 필터 → 펌 필터`에서 모든 펌 게시글을 삭제·차단·보류하는 정책을 따로 설정할 수 있습니다.
  - `기본 탐색 설정 → 탐색 범위 및 속도 설정`에서 현재 탐색 범위 안의 변경 없는 펌 글을 매사이클 다시 검사할 수 있습니다.
- 스냅샷에서 당시의 펌 원문을 확인할 수 있습니다.
  - 최초·최신·차단 스냅샷에 당시 확인한 원문의 제목·작성정보·본문·이미지·댓글 모양과 확인 상태를 보존합니다.
  - 백업에서 복원한 구버전 스냅샷의 최초·최신 구분과 누락 파일 복구를 더 안정적으로 처리합니다.
- AI와 장시간 운영 안정성을 개선했습니다.
  - 서로 다른 갤러리의 글을 정확히 구분하고, 글이나 댓글이 바뀌면 이전 AI 판단을 그대로 적용하지 않습니다.
  - AI 또는 펌 원문 확인이 잠시 실패해도 다음 검사에서 다시 확인할 수 있습니다.
- 설정 화면을 개선했습니다.
  - 개발자 설정을 숨겨진 연속 누르기 대신 `개발자 설정 열기` 버튼으로 엽니다.
  - 펌 반복 검사 설정을 탐색 범위 및 속도 화면으로 옮겼습니다.
  - 특수문자 필터를 `@` 모양 아이콘으로 구분했습니다.
- 상세 매뉴얼, 패치노트, 초심자 가이드를 1.5.0 기준으로 갱신했습니다.
  - 펌 원문 검사, 펌 전체 차단, 매사이클 재검사의 차이를 실제 사용 예시로 설명합니다.

### 검증
- `./gradlew.bat clean testDebugUnitTest testReleaseUnitTest lintDebug lintVitalRelease assembleRelease` 통과
- Debug/Release 단위 테스트 각각 265개 통과, 실패·오류·건너뜀 0개
- APK `versionCode 159` / `versionName 1.5.0` 확인
- APK Signature Scheme v2 및 기존 1.4.5와 동일한 인증서 지문 확인
- DEX 3개 검증 통과, 최대 호출 인자 246, 주 DEX 메서드 여유 187
- Google Drive 스테이징 재다운로드 파일의 크기와 SHA-256이 로컬 APK와 일치
- 매뉴얼·패치노트·초심자 가이드의 탭·번호·문단 스타일·글머리표와 PDF 렌더링 검증
...
- versionName: `1.5.0`
- APK: `완장봇_v1.5.0.apk`
- SHA-256: `66a3156c1d7121be55bb9bb6fac53149a22bf3e823760e442a8bb21f7c6d21b2`
- 파일 크기: `12,945,102 bytes`
- Google Drive file ID: `16PCv3zIuFR6FP93DsWCWqxE7F16sunzo`
- Google Drive link: https://drive.google.com/file/d/16PCv3zIuFR6FP93DsWCWqxE7F16sunzo/view?usp=drivesdk
...
### 주요 변화
- 스냅샷 뷰어에서 같은 디시콘이 연속 2개 이상 있어도 중복 제거하지 않고 모두 표시하도록 수정했습니다.
- 연속 디시콘은 같은 줄에 간격 없이 붙여서 표시되도록 개선했습니다.
- 스냅샷 목록 스와이프 액션의 `원문 열기` 문구를 `링크`로 변경하고, 원문 WebView에서 뒤로가기는 현재 원문 창만 닫도록 수정했습니다.
- 이미지 alt 추출 범위를 게시글 본문 영역으로 제한해 사이트 UI 이미지가 섞이지 않도록 수정했습니다.
- 이미지 alt 미리보기에 DCInside Referer/User-Agent 헤더를 적용해 본문 이미지 미리보기 로딩을 개선했습니다.
- 이미지 차단 목록에 게시글 주소로 찾아 넣기 기능을 통합하고, 직접 alt값 추가와 함께 한 화면에서 관리하도록 개선했습니다.
- 디시콘 필터를 이미지 필터 내부가 아닌 별도 상세 세션으로 분리해 목록 구분감을 강화했습니다.
...
### 주요 변화
- 세션 만료 오판/자동 로그인 복구 루프 핫픽스
  - 사이클 시작 직후 세션 복구가 성공하면 다음 사이클로 넘기지 않고 같은 사이클에서 검사를 계속 진행합니다.
  - 자동 로그인으로 새 세션을 확보했을 때 기존 저장 쿠키와 무조건 병합하지 않고 새 로그인 쿠키 기준으로 저장해 오래된 쿠키 조각이 남는 문제를 줄였습니다.
  - 세션 유효성 확인을 `로그아웃` 텍스트 단일 기준에서 보강해, 로그인 페이지로 확정되지 않고 주요 세션 쿠키가 있으면 유효 세션으로 인정합니다.
- 증상 대응: 매 사이클마다 “자동 로그인 성공”만 반복되고 실제 게시글/댓글 검사가 진행되지 않는 상태 완화.
...
- APK: `완장봇_v1.4.1-beta1.apk`
- SHA256: `E4E956C0589CE4391CC13F2474F3A9E69221B538B5E56BD441B46DD7D58642B2`
- Google Drive file ID: `1QAutcvazVgtQBGF_E3g3qMBY5Fcea7qP`
...
- AI compact 응답의 P/C 타입 혼동 방지 프롬프트 강화
  - 게시글 번호는 반드시 `P` 줄에만 출력
  - 댓글 번호는 반드시 `C` 줄에만 출력
  - 게시글 번호를 `C|...`로 출력하거나 댓글 번호를 `P|...`로 출력하는 것 금지
- AI 배치 내 문맥 간섭 방지 프롬프트 강화
  - 배치는 처리 효율용 포장일 뿐 여러 글을 한 사건/대화/흐름으로 묶어 해석하지 않도록 명시
  - 한 글/댓글에서 나온 금지 주제·분위기·결론을 다른 항목에 전염시키지 않도록 제한
  - 현재 항목 텍스트 안에 `user_prompt` 위반 근거가 직접 없으면 허용하도록 보강

### 검증
- `./gradlew.bat assembleRelease` 성공
- APK: `완장봇_v1.4.0-beta15.apk`
- SHA256: `B9AB78D4DE3082F5866462267B8D5D1B409EF4707FDA6B92F338575E07C3FE47`
- Google Drive file ID: `19mQg0Cuhs53i35D2oErlryXGf7F4Tku4`

### 다음 작업 메모
- beta15에서 확인할 것: `요청에 없는 comment_id=게시글번호` 로그가 줄어드는지, 한 배치에서 여러 정상 글이 한꺼번에 보류/삭제 대상으로 번지는 현상이 줄어드는지.

## 1.4.0 beta14 (최신 업로드본)

### 주요 변화
- AI 댓글 즉시 집행 경로의 차단 기록 사유 수정
  - 기존 hardcoded `AI 댓글 배치 즉시 집행` 대신 AI가 반환한 개별 판단 이유를 사용
- AI 기반 게시글/댓글 차단·보류 기록 사유에 `[AI]` prefix 적용
  - 예: `[AI] 여장관련`, `[AI] 동성애`
  - 기존 `[AI]`가 이미 붙은 경우 중복 prefix 방지
- 삭제된 댓글 안내문을 AI 배치 입력에서 제외
  - `해당 댓글은 삭제되었습니다.` 등 삭제 placeholder/빈 댓글을 AI 판단 대상에서 제거
- AI 시스템 프롬프트 정책 경계 강화
  - `user_prompt`를 유일한 차단 정책으로 명시
  - `user_prompt`에 없는 사유로 차단하지 않도록 제한
  - 일반 성적 내용/욕설/취향/정체성/논쟁성은 사용자 지침이 직접 금지하지 않으면 허용하도록 보강
  - 입력에 없는 ID 생성/출력 금지 문구 추가

### 검증
- `./gradlew.bat assembleRelease` 성공
- APK: `완장봇_v1.4.0-beta14.apk`
- SHA256: `2FC0ADE0B51BF9DC8B7CE10791AC97757FDDD69FB768F604EC3A207F843241E9`
- Google Drive file ID: `1lRiuGAcojHPk-gfV_dzJqykzsf0RTxWd`

### 다음 작업 메모
- beta14에서 확인할 것: AI 댓글 차단 기록이 `[AI] <짧은 이유>`로 저장되는지, 삭제 댓글이 AI 대상에서 빠지는지, user_prompt 밖의 넓은 성적/논쟁성 사유로 보류하지 않는지.
