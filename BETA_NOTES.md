...
### 주요 변화
- 전체 스냅샷(`모두`) 저장 후 UI에서 `스냅샷이 없음`으로 보일 수 있던 경로를 보강
- 차단 스냅샷은 정상인데 전체 스냅샷만 비어 보인다는 제보를 기준으로 일반 스냅샷 저장 결과의 DB 반영을 정리
- 전체 스냅샷 저장 성공/실패 로그를 추가해 실제 저장 여부를 더 쉽게 확인할 수 있도록 보강

### 기술 포인트
- `saveSnapshotFromDoc(postDoc, commentsArray)` 반환값을 `generalSnapshotPath`로 직접 받아 사용하도록 수정
- 전체 스냅샷 저장 성공 시 `dbSnapshotPath`에 반영하고 `postDao().updateSnapshotPath(...)`를 호출하도록 정리
- 저장 경로가 없으면 `[스냅샷][전체] 저장 실패 또는 경로 없음` 로그를 남기도록 추가
...
### 주요 변화
- AI 배치 하네스에 묶음 내 항목 간 맥락 전이 금지 규칙을 추가
- 자동 로그인 구현을 현재 DC 로그인 페이지 구조(`sign.dcinside.com/login` → `login/member_check`) 기준으로 재작성
- AI 배치 간섭 및 자동 로그인 실패 문제를 함께 검증하기 위한 베타

### 기술 포인트
- `AiFilter.kt` fixed prompt에 각 게시글/댓글을 서로 완전히 독립적으로 판단하라는 규칙을 추가
- 유사 표현/연상/다른 항목 결론 전이 금지 규칙을 하네스 차원에서 명시
- `performAutoLogin(...)`를 최신 로그인 폼/hidden input/action 구조 기반으로 재구성
- `assembleDebug` 빌드 성공으로 현재 수정 코드가 컴파일 가능함을 확인
...
### 주요 변화
- AI 댓글 즉시집행 경로에서도 작성일시가 비어 저장되던 문제를 보강
- AI 게시글/댓글 차단 경로 모두 원본 페이지 기준으로 작성일시를 다시 확보하도록 정리
- AI 차단 기록의 작성일시 누락 문제를 일반 필터 차단 수준으로 맞추기 위한 보강 단계 진행

### 기술 포인트
- AI 댓글 차단 시 detail page + comment API를 다시 조회해 해당 댓글의 `reg_date/date`를 `creationDate` fallback으로 사용
- AI 게시글/댓글 차단 경로 모두 빈 날짜 문자열 대신 원본 기준 작성일시를 저장하도록 보강
- `assembleDebug` 빌드 성공으로 현재 수정 코드가 컴파일 가능함을 확인
...
### 주요 변화
- AI 댓글 차단 스냅샷 저장 경로를 기존 일반 스냅샷 저장 계열과 더 가깝게 연결하는 브리지 로직을 추가
- 기존 대규모 스냅샷 공용화 시도 중 깨졌던 `BotService.kt`를 복구하고, 빌드 가능한 상태에서 안전한 최소 변경만 반영
- AI 댓글 차단 스냅샷 생성 시 기존 저장 규칙(파일명/전문가 모드 기준)을 최대한 재사용하도록 보정

### 기술 포인트
- `captureCommentBlockSnapshot(...)`에서 직접 파일을 쓰던 경로 대신 compat helper를 거쳐 기존 저장 방식과 접점을 만들었음
- 완전한 `saveSnapshotFromDoc(...)` 공용화는 다음 단계 과제로 남기고, 우선 실기기 검증 가능한 beta를 확보하는 데 집중
- `assembleDebug` 빌드 성공으로 현재 코드가 다시 안정 상태임을 확인
...
### 주요 변화
- AI 배치 즉시집행 경로에서도 차단 스냅샷이 저장되도록 보강
- AI 즉시집행 차단 로그 문구를 정리해 `AI 필터 차단 차단!`처럼 어색한 중복 표현이 나오지 않도록 수정
- AI 차단 성공 후 운영 로그/증거 보존 품질을 일반 차단 흐름에 가깝게 맞춤

### 기술 포인트
- `BotService.kt`의 즉시집행 `handleBadPost(...)` 호출에 `captureBlockSnapshot(...)` 기반 `saveSnapshotFn` 연결
- `buildPostBlockPresentation()`의 AI 분기 우선순위를 조정해 AI 차단 로그를 전용 문구(`AI 배치 차단!`)로 정리
